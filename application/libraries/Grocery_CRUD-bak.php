		...
		if(!empty($this->edit_fields))
			foreach($this->edit_fields as $field_object)
			{
				...
				if(!isset($types[$field_name]))//Doesn't exist in the database? Create it for the CRUD
				{
					...
					if ( $this->required_fields == null) $this->required_fields = array();
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_true_false_input($field_info,$value)
	{
		...
		$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/jquery.uniform.js');
	...
	protected function get_text_input($field_info,$value)
	{
		...
		{
			$input = "<textarea class='form-control' rows='5' id='field-{$field_info->name}' name='{$field_info->name}'>$value</textarea>";
		}
		return $input;
	}
	...
	protected function get_enumradio_input($field_info,$value)
	{
		//$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		...
		foreach($options_array as $option)
		{
			$selected = !empty($value) && $value == $option ? "checked='checked'" : '';
			$input .= "<input name='{$field_info->name}' value='$option' $selected type='radio'> $option &nbsp; ";
		}
		...
		if($this->columns_checked === false)
		{
			...
			foreach($this->columns as $col_num => $column)
			{
				...
				if(isset($this->relation[$column]))
				{
					...
					$col = preg_replace(array('/{/','/}/'), array('',''), $this->relation[$column][2]);
					
					$new_column = $this->_unique_field_name($this->relation[$column][0]);
					$input_name = $this->_unique_join_name($this->relation[$column][0]) .'.'.$col;
				...
				if($input_name != null)
					$this->columns[$col_num]->input_name = $input_name;
				else 
					$this->columns[$col_num]->input_name = $this->get_table().'.'.$column;
