...
class Grocery_crud_model  extends CI_Model  {
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
		...
		if($this->field_exists('deleted_at',$related_table)) {
    ...
    function db_relation_n_n_update($field_info, $post_data ,$main_primary_key)
    {
		...
		if ($this->field_exists('deleted_at', $field_info->relation_table )) {
			$nowtime = date('Y-m-d H:i:s');
			$this->db->update($field_info->relation_table, array('deleted_at' => $nowtime));
		} else {
    ...
    function db_delete($primary_key_value)
    {
		...
		if ($this->field_exists('deleted_at', $this->table_name)) {
			$nowtime = date('Y-m-d H:i:s');
			$this->db->where(array( $primary_key_field => $primary_key_value));
			$this->db->update($this->table_name, array('deleted_at' => $nowtime));
		} else {
